...
func addPortToSwitch(sw string, port string) {
	//command := exec.Command("ovs-vsctl", "add-port", sw, port, "-- set Interface", port, "type=internal")
	command := exec.Command("ovs-vsctl", "add-port", sw, port)
	run(command)
}

func addPortToSwitchWithPortNum(sw string, port string, portNum int) {
	//command := exec.Command("ovs-vsctl", "add-port", sw, port, "--", "set", "interface", sw, "ofport="+strconv.FormatInt(int64(portNum), 10), "type=internal")
