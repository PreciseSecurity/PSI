...
public class PSIEventHandler {
    ...
    // a threshold for 10ms
    public int rate_threshold = 100000;
        ...
        for (PSIMessage msg : msgs){
            ...
            for (int i = 0; i<alertmsglist.size(); i++)
            {
                ...
                if (msrc.find() && mdst.find() && me.find())
                {
                    ...
                    PSIPolicyManager.getInstance().handleEvent(new PSIEvent("Event1", "10.1.0.1", "10.2.0.1", "Event1"));
                }

                /* The module for response time and scale out */
                if ((trans_num%50000)==1 && ((c_time - s_time) != 0))
                {
                    float receive_rate = (((float)trans_num)*1000/(c_time-s_time));
                    System.out.print("Receive rate ="+(((float)trans_num)*1000/(c_time-s_time))+"\n");

                    if (receive_rate > rate_threshold)
                    {
                        PSIDAGModifier.getInstance().execCMD("sh /psi/repo/scale_out.sh");
                    }
                }
