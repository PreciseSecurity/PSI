...
public class CoreRouter extends OFNode {
	...
	public boolean addFlowByTraffic(String src, String dst, String mask, String outPort, short priority) {
		if(! this.addFlowRule(this.initailRuleByTraffic(src, dst, mask, outPort, priority))){
			return false;
		}
        return true;
    }

	public boolean addFlowByTraffic(String src, String dst, String mask, String inPort, String outPort, short priority) {
		if(! this.addFlowRule(this.initailRuleByTraffic(src, dst, mask, inPort, outPort, priority))){
			return false;
		}
        return true;
    }
	public boolean addFlowByDstHost(int dstHost, String src, String dst, String mask, String inPort, short priority) {
        return true;
    }
	...
	*/
	...
	private Flow initailRuleByTraffic(String src, String dst, String mask, String outPort, short priority) {
		Flow flow = new Flow();
		Match match = new Match();
		List<Action> actions = new ArrayList<Action>();
		match.setField(MatchType.DL_TYPE, (short) 0x0800);
		match.setField(MatchType.NW_TOS, (byte)0);
		try {
			match.setField(MatchType.NW_DST, InetAddress.getByName(src),
					InetAddress.getByName(mask));
			match.setField(MatchType.NW_DST, InetAddress.getByName(dst),
					InetAddress.getByName(mask));
		} catch (UnknownHostException e) {
	...
	private Flow initailRuleByTraffic(String src, String dst, String mask, String inPort, String outPort, short priority) {
		Flow flow = new Flow();
		Match match = new Match();
		List<Action> actions = new ArrayList<Action>();
		match.setField(MatchType.DL_TYPE, (short) 0x0800);
	...
	private Flow initailRuleByDstHost(int dstHost, String src, String dst, String mask, String inPort, short priority) {
		Flow flow = new Flow();
		Match match = new Match();
		List<Action> actions = new ArrayList<Action>();
		match.setField(MatchType.DL_TYPE, (short) 0x0800);
		...
		// actions.add(new Output(this.connectors.get(outPort)));
		NodeConnector outPort = this.forwardingTable.get(dstHost);
		if(outPort == null){
			logger.info(this.node.getNodeIDString() + " doesn't have fw table for " + dstHost);
		}
		actions.add(new Output(outPort));
